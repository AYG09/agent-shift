...
`REQ-PAR-006`: 새 저장소는 **빈 이력에서 시작한다** (확정, 2026-08-27). 원본 커밋 이력을
가져오지 않으며 `git filter-repo` 같은 도구를 쓰지 않는다.

근거: 작업 경위와 설계 판단은 이미 문서 세트(`docs/sop-member-context-redesign/**`,
`docs/sop-standalone-extraction/**`, skill의 reference 5개)로 존재하고 그 문서들이 함께
이관된다. 원본 이력에는 `/flow` 커밋이 섞여 있어 필터링해도 완전하지 않으며, 잘못 남은
이력이 사내망 저장소에서 오히려 감사 대상이 된다.

`INT-PAR-002`: 이력이 없으므로 `git blame`으로 "이 줄이 왜 이런가"를 추적할 수 없다. 이
저장소는 그 상실을 이미 상쇄하는 관행을 갖고 있다 — `QUALITY_CONVENTIONS.md` 축 1이
"왜 이렇게 되어 있는가"를 코드 옆 docstring으로 남기도록 요구하고, 도메인 모듈 대부분이 그
형태로 작성돼 있다. **이관 중 그 docstring을 줄이거나 요약하지 않는다.** 이력이 없는
저장소에서는 그것이 유일한 설계 근거다.
