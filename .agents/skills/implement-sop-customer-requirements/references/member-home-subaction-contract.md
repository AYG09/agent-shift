...
- Confirmed project direction (2026-08): the DEFAULT decomposition expectation is 2–3 Sub Actions per Activity — a 14-Activity Task should produce roughly 28–42 business nodes. Generation capacity (`computeSubActionCapacity`) floors `minSteps` at 2× the Activity count and gives 3×-band headroom for `maxSteps`; the generation runner asks for a repair when an Activity comes back with only one Sub Action, then downgrades a surviving case to a warning (never a hard failure or a confirm-boundary rule) because a genuinely atomic Activity must not be force-split. Derive the actual count from independently executable actions and their dependencies; never present a specific generated count as customer-mandated.
