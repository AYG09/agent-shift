...
- repair 호출은 퇴행 반복 루프에 대비해 방어한다: (1) 와이어 스키마의 `type` 필드는 enum으로 제한해 제약 디코딩이 자유 문자열 반복 폭주를 차단하고, (2) repair 프롬프트에는 직전 응답 전체 JSON(10만+ 자) 대신 구조 요약(`buildSopStructuralDigest`)만 첨부하며, (3) 실패한 repair 생성은 1회 재시도(`generateSopRepairWithRetry`)한 뒤에야 결정론적 fallback/오류 경로로 넘어간다. 이 세 방어가 없으면 단 한 번의 퇴행 생성이 유일한 repair 기회를 소모해 즉시 400이 된다.
