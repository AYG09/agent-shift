                ...
                <div className="space-y-4 py-4">
                    <Card className="bg-slate-800/50 border-slate-700">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm text-slate-300">BYOK (Bring Your Own Key)</CardTitle>
                    ...
                    <div className="text-xs text-slate-500 space-y-1">
                        ...
                        <p>• <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener" className="text-indigo-400 underline">Google AI Studio</a>에서 키를 발급받을 수 있습니다.</p>
                    </div>
                </div>

                <DialogFooter className="flex justify-between">
                    {apiKey && (
                        <Button variant="destructive" size="sm" onClick={handleClear} className="mr-auto">
                            🗑️ 삭제
                        </Button>
                    )}
                    <div className="flex gap-2">
                        <Button variant="outline" onClick={() => setOpen(false)} className="border-slate-600 bg-white text-slate-900 hover:bg-slate-100">
                            취소
                        </Button>
                        <Button onClick={handleSave} disabled={!inputKey.trim()} className="bg-indigo-600 hover:bg-indigo-500">
                            저장
                        </Button>
                    </div>
                </DialogFooter>
