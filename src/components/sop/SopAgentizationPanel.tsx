...
import { AI_APPLICATION_MODES, getAgentizationModeForStep } from '@/lib/sop-agentization';
        ...
        <div className="flex h-full min-h-0 flex-col border-l border-zinc-200 bg-white">
            ...
            <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4 text-xs">
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 rounded-md bg-zinc-100 p-1"><button type="button" onClick={() => setAgentizationScope('workflow')} className={`rounded px-2 py-1.5 font-semibold ${review.scope === 'workflow' ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500'}`}>전체 워크플로우</button><button type="button" onClick={() => setAgentizationScope('steps')} className={`rounded px-2 py-1.5 font-semibold ${review.scope === 'steps' ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500'}`}>선택 단계</button></div>
                </div>

                {review.scope === 'steps' && <div><div className="mb-1.5 flex items-center justify-between"><label className="font-semibold text-zinc-800">검토 대상 단계</label><span className="text-[10px] text-zinc-500">시작·종료 제외</span></div><div className="divide-y divide-zinc-100 overflow-hidden rounded-md border border-zinc-200">{document.steps.filter((step) => eligibleIds.includes(step.id)).map((step, index) => { const checked = review.stepIds.includes(step.id); return <div key={step.id} className="flex items-center gap-2.5 bg-white px-3 py-2 hover:bg-zinc-50"><input id={`agent-step-${step.id}`} type="checkbox" checked={checked} onChange={() => toggleAgentizationStep(step.id)} className="h-3.5 w-3.5 rounded-sm text-indigo-600 focus:ring-indigo-500" /><label htmlFor={`agent-step-${step.id}`} className="min-w-0 flex-1 cursor-pointer"><span className="mr-1.5 font-mono text-[10px] text-zinc-400">{String(index + 1).padStart(2, '0')}</span><span className="font-medium text-zinc-800">{step.title}</span></label><button type="button" onClick={() => selectStep(step.id)} className="text-[10px] font-medium text-indigo-600 hover:text-indigo-800">상세</button></div>; })}</div></div>}

                {selectedIds.length > 0 && <div><div className="mb-1.5 flex items-center justify-between"><label className="font-semibold text-zinc-800">일괄 기본값</label><span className="text-[10px] text-zinc-500">선택 노드에 적용 후 개별 수정 가능</span></div><div className="grid grid-cols-1 gap-1.5">{AI_APPLICATION_MODES.map((mode) => { const Icon = MODE_ICONS[mode.id]; return <button key={mode.id} type="button" onClick={() => setAgentizationDefaultMode(mode.id)} className="flex items-center gap-2 rounded-md border border-zinc-200 bg-white px-2.5 py-1.5 text-left hover:border-indigo-300 hover:bg-indigo-50/40"><Icon className="h-3.5 w-3.5 text-indigo-600" /><span className="text-[11px] font-semibold text-zinc-800">{mode.label}</span><span className="ml-auto text-[10px] text-zinc-500">일괄 적용</span></button>; })}</div></div>}

                {selectedIds.length > 0 && <div><div className="mb-1.5 flex items-center justify-between"><label className="font-semibold text-zinc-800">노드별 AI 적용 방식</label><span className="text-[10px] text-zinc-500">예외를 직접 지정</span></div><div className="divide-y divide-zinc-100 overflow-hidden rounded-md border border-zinc-200">{selectedIds.map((id) => { const step = document.steps.find((item) => item.id === id); if (!step) return null; const mode = getAgentizationModeForStep(document, id); return <div key={id} className="bg-white px-3 py-2.5"><div className="mb-1.5 flex items-center justify-between gap-2"><button type="button" onClick={() => selectStep(id)} className="min-w-0 truncate text-left text-[11px] font-semibold text-zinc-800"><span className="mr-1.5 font-mono text-[10px] text-zinc-400">{String(document.steps.indexOf(step) + 1).padStart(2, '0')}</span>{step.title}</button>{mode && <span className="text-[10px] text-emerald-600">지정됨</span>}</div><select value={mode || ''} onChange={(event) => setAgentizationStepMode(id, event.target.value as SopAiApplicationMode)} className="w-full rounded-md border border-zinc-300 bg-zinc-50 px-2 py-1.5 text-[11px] font-medium text-zinc-800 focus:border-indigo-500 focus:outline-none"><option value="">적용 방식 선택</option>{AI_APPLICATION_MODES.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></div>; })}</div></div>}

                <div><label className="mb-1.5 block font-semibold text-zinc-800">판단 근거 <span className="font-normal text-zinc-400">(선택)</span></label><textarea rows={2} value={review.note || ''} onChange={(event) => setAgentizationNote(event.target.value)} placeholder="예: 반복 규칙은 명확하나 최종 승인에는 사람이 필요" className="w-full resize-none rounded-md border border-zinc-300 bg-zinc-50 px-3 py-2 text-xs leading-5 text-zinc-800 placeholder:text-zinc-400 focus:border-indigo-500 focus:bg-white focus:outline-none" /></div>
            ...
            <div className="shrink-0 border-t border-zinc-200 bg-white p-4"><button type="button" onClick={handleConfirm} className="flex w-full items-center justify-center gap-1.5 rounded-md bg-indigo-600 px-3 py-2.5 text-xs font-semibold text-white transition-colors hover:bg-indigo-700"><CheckCircle2 className="h-4 w-4" /> Agent화 검토 확정</button></div>
        </div>
    );
};
