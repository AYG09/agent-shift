...
import { getAgentizableDemoStepIds, useSopDemoStore, scenarioInfo } from '@/lib/sop-demo-store';
...
type NodeData = { step: DemoStep; number: number; mode: DemoMode; onSelect: (id: string) => void; agentCandidate: boolean; agentizationConfirmed: boolean };
...
function DemoNode({ data }: NodeProps<Node>) {
    const { step, number, mode, onSelect, agentCandidate, agentizationConfirmed } = data as unknown as NodeData;
    ...
    return (
        <button
            ...
            className={`relative block text-left text-slate-800 ${agentCandidate ? 'drop-shadow-[0_0_0.35rem_rgba(124,58,237,0.35)]' : ''}`}
            style={{ width, height }}
        ...
        >
            ...
            >
                ...
                {agentCandidate && <span className={`mt-1 inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[9px] font-bold ${agentizationConfirmed ? 'bg-violet-600 text-white' : 'bg-violet-100 text-violet-700'}`}><Bot size={10}/>{agentizationConfirmed ? 'Agent화 확정' : 'Agent화 후보'}</span>}
...
export function DemoWorkspace() {
    ...
    const [flow, setFlow] = useState<ReactFlowInstance | null>(null); const [notice, setNotice] = useState(''); const [edgeTarget, setEdgeTarget] = useState(''); const [agentPanelOpen, setAgentPanelOpen] = useState(false);
    ...
    const agentCandidateIds = useMemo(
        () => new Set(store.agentizationScope === 'workflow' ? agentizableStepIds : store.agentizationStepIds.filter((id) => agentizableStepIds.includes(id))),
        [agentizableStepIds, store.agentizationScope, store.agentizationStepIds]
    );
    ...
    const nodes = useMemo<Node[]>(() => document?.steps.map((step, index) => ({ id: step.id, type: 'demo', position: step.position, data: { step, number: index + 1, mode: store.mode, onSelect: store.selectStep, agentCandidate: agentCandidateIds.has(step.id), agentizationConfirmed: !!store.agentizationConfirmedAt }, selected: store.selectedStepId === step.id })) || [], [document, store.mode, store.selectedStepId, store.selectStep, store.agentizationConfirmedAt, agentCandidateIds]);
    ...
    return <div className="absolute inset-0 z-50 flex justify-end bg-slate-950/20" role="dialog" aria-modal="true" aria-label="Agent화 가능 업무 확정">
        <aside className="flex h-full w-full max-w-[420px] flex-col border-l border-slate-200 bg-white shadow-2xl">
            ...
            <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
                ...
                <section><h3 className="text-xs font-bold text-slate-700">2. AI 역할</h3><div className="mt-2 grid grid-cols-2 gap-2"><button onClick={() => store.setAgentizationMode('automation')} className={`rounded-lg border p-3 text-left ${store.agentizationMode === 'automation' ? 'border-violet-500 bg-violet-50' : 'border-slate-200'}`}><strong className="text-xs">AI 대체</strong><p className="mt-1 text-[10px] leading-4 text-slate-500">규칙 기반 실행·처리를 Agent가 수행</p></button><button onClick={() => store.setAgentizationMode('collaboration')} className={`rounded-lg border p-3 text-left ${store.agentizationMode === 'collaboration' ? 'border-violet-500 bg-violet-50' : 'border-slate-200'}`}><strong className="text-xs">AI 협업</strong><p className="mt-1 text-[10px] leading-4 text-slate-500">사람 판단을 지원하고 검토 지점을 유지</p></button></div></section>
                {store.agentizationScope === 'steps' && <section><div className="flex items-center justify-between"><h3 className="text-xs font-bold text-slate-700">3. Agent화 후보 단계</h3><span className="text-[11px] font-bold text-violet-700">{selectedIds.length}개 선택</span></div><div className="mt-2 space-y-1.5">{document.steps.filter((step) => agentizable.includes(step.id)).map((step, index) => { const checked = selectedIds.includes(step.id); return <button key={step.id} onClick={() => store.toggleAgentizationStep(step.id)} className={`flex w-full items-center gap-2 rounded-lg border p-2.5 text-left ${checked ? 'border-violet-400 bg-violet-50' : 'border-slate-200 hover:bg-slate-50'}`}>{checked ? <CheckSquare size={16} className="shrink-0 text-violet-600"/> : <Square size={16} className="shrink-0 text-slate-400"/>}<span className="text-[10px] text-slate-400">{String(index + 1).padStart(2, '0')}</span><span className="min-w-0 flex-1 truncate text-xs font-semibold">{step.title}</span></button>; })}</div></section>}
                <section><label className="block text-xs font-bold text-slate-700">4. 판단 근거 <span className="font-normal text-slate-400">(선택)</span><textarea value={store.agentizationNote} onChange={(event) => store.setAgentizationNote(event.target.value)} rows={3} placeholder="예: 반복적으로 접수되는 지원서의 자격 요건 확인은 규칙 기반으로 자동화 가능" className="mt-2 w-full resize-none rounded-lg border border-slate-300 p-2.5 text-xs leading-5 outline-none focus:border-violet-500"/></label></section>
                {confirmed && <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-xs leading-5 text-emerald-800"><strong>확정됨</strong> · {selectedIds.length}개 업무를 {store.agentizationMode === 'automation' ? 'AI 대체' : 'AI 협업'} 후보로 저장했습니다. SOP 내용이나 판단을 수정하면 다시 확정해야 합니다.</div>}
            </div>
            <div className="border-t border-slate-200 bg-white p-5"><div className="mb-3 rounded-lg bg-slate-50 p-3 text-[11px] leading-5 text-slate-600"><strong className="text-slate-800">선택 결과</strong> · {store.agentizationScope === 'workflow' ? '전체 워크플로' : `${selectedIds.length}개 선택 단계`} / {store.agentizationMode === 'automation' ? 'AI 대체(Agent 실행)' : 'AI 협업(Agent 보조)'}</div><button onClick={() => { const result = store.confirmAgentization(); onNotice(result.message); }} className="flex w-full items-center justify-center gap-2 rounded-lg bg-violet-600 py-3 text-sm font-bold text-white hover:bg-violet-700"><Bot size={16}/>{confirmed ? 'Agent화 판단 다시 확정' : 'Agent화 가능 여부 확정'}</button></div>
        </aside>
    </div>;
}
