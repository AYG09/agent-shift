        ...
        >
            ...
            >
                <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500">
                    <span className="rounded bg-slate-100 px-1.5 py-0.5">{formatStepNumber(number)}</span>
    ...
    return <div className="absolute inset-0 z-50 flex justify-end bg-slate-950/20" role="dialog" aria-modal="true" aria-label="Agent화 가능 업무 확정">
        <aside className="flex h-full w-full max-w-[420px] flex-col border-l border-slate-200 bg-white shadow-2xl">
            ...
            <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
                ...
                {store.agentizationScope === 'steps' && <section><div className="flex items-center justify-between"><h3 className="text-xs font-bold text-slate-700">2. Agent화 후보 단계</h3><span className="text-[11px] font-bold text-violet-700">{selectedIds.length}개 선택</span></div><div className="mt-2 space-y-1.5">{document.steps.filter((step) => agentizable.includes(step.id)).map((step, index) => { const checked = selectedIds.includes(step.id); return <button key={step.id} onClick={() => store.toggleAgentizationStep(step.id)} className={`flex w-full items-center gap-2 rounded-lg border p-2.5 text-left ${checked ? 'border-violet-400 bg-violet-50' : 'border-slate-200 hover:bg-slate-50'}`}>{checked ? <CheckSquare size={16} className="shrink-0 text-violet-600"/> : <Square size={16} className="shrink-0 text-slate-400"/>}<span className="text-[10px] text-slate-400">{formatStepNumber(index + 1)}</span><span className="min-w-0 flex-1 truncate text-xs font-semibold">{step.title}</span></button>; })}</div></section>}
